...
|-----------|------|--------|---------|--------|-------|
| `page`    | `number` | No | Page number (default: 1) | ✅ Preserved | Added default value of 1 if not provided |
| `pageSize`| `number` | No | Items per page (default: 10) | ✅ Preserved | Added default value of 10 if not provided |
| `search`  | `string` | No | Search term for filtering scans | ✅ Preserved | Pass-through to backend; optional query string |
| `rat`     | `string` | No | Filter by RAT (Radio Access Technology); e.g., 'LTE', 'NR' | **ADDED** | If null or `'ALL'`, no filter applied; omitted from query entirely |
...
export interface GetScansParams {
  ...
  pageSize?: number
  search?: string
  rat?: string // Filter by RAT; if null or 'ALL', no filter is applied
}
...
export async function getScans(params?: GetScansParams): Promise<ScanPaginated> {
  ...
  // Only include rat filter if it's provided AND is NOT 'ALL' or empty
